...
BNO08xROS::BNO08xROS() : Node("bno08x_ros")
{
  ...
  if (this->imu_rate_ < this->magnetic_field_rate_) {
    this->poll_timer_ =
        this->create_wall_timer(std::chrono::milliseconds(1000 / this->magnetic_field_rate_),  // Hz to ms
                                std::bind(&BNO08xROS::poll_timer_callback, this));
  } else {
    this->poll_timer_ = this->create_wall_timer(std::chrono::milliseconds(1000 / this->imu_rate_),  // Hz to ms
                                                std::bind(&BNO08xROS::poll_timer_callback, this));
  }
...
void BNO08xROS::init_sensor()
{
  ...
  {
    bno08x_ =
        new BNO08x(comm_interface_,
                   std::bind(&BNO08xROS::sensor_callback, this, std::placeholders::_1, std::placeholders::_2), this);
  }
  ...
  if (publish_magnetic_field_)
  {
    if (!this->bno08x_->enable_report(SH2_MAGNETIC_FIELD_CALIBRATED, 1000000 / this->magnetic_field_rate_))
    {  // Hz to us
